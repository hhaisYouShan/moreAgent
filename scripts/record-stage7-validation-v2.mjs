...
assertCondition(workflow.headBranch === 'ci/bossresume-stage7-v2', `Unexpected Stage 7 branch: ${workflow.headBranch}`);
...
assertCondition(['SUCCESS', 'FAILURE', 'CANCELLED', 'TIMED_OUT', 'ACTION_REQUIRED'].includes(machineResult), `Unsupported validation result: ${machineResult}`);
...
console.log(`[stage7] workflow result: ${machineResult}`);
console.log(`[stage7] report status: ${report.status}`);
console.log(`[stage7] open Blocking/Major: ${blockingOrMajor.length}`);
console.log(`[stage7] required artifacts present: ${requiredPresent}`);
